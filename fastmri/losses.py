...
class AttenuatedMaskedL1Loss(torch.nn.MSELoss):
    ...
    def forward(
        ...
        return F.attenuated_masked_l1_loss(input, target, mask, log_b, reduction=self.reduction)


class MaskedL2Loss(torch.nn.MSELoss):
    r"""Creates a criterion that measures the mean squared error (squared L2 norm) between
    each element in the input :math:`x` and target :math:`y`.
    ...
    def __init__(self, reduction: str = 'mean') -> None:
        assert reduction in ['mean', 'none']
        super(MaskedL2Loss, self).__init__(None, None, reduction)

    def forward(self, input: torch.Tensor, target: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        return F.masked_l2_loss(input, target, mask, reduction=self.reduction)
